...
class CustomersOrdersScreen extends StatefulWidget {
  final SearchFilter? searchFilter;
  const CustomersOrdersScreen({this.searchFilter, super.key});

  @override
  State<CustomersOrdersScreen> createState() => _CustomersOrdersScreenState();
}

class _CustomersOrdersScreenState extends State<CustomersOrdersScreen> {
  ...
  Future<void> getProducts({String? searchTerm}) async {
    ...
    products = await provider.fetchCustomersOrders(context: context,showLoader: false);
    if(mounted){
      setState(() {
        loading = false;
      });
                    ...
                    onTap: () {
                      Navigator.push(context, CupertinoPageRoute(
                        builder: (context) => OrderDetailsScreen(
                          orderId: order?.orderId ?? order?.order?.id ?? order?.id,
                        ),
                      ));
                    },
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text("Total Amount : ${order?.order?.totalAmount}", style: const TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w400,
                            color: Colors.grey,
                          )),
                          const SizedBox(height: 5),
                          Text("Shipping Address : ${order?.order?.shippingAddress}", style: const TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w400,
                            color: Colors.grey,
                          ),maxLines: 1,overflow: TextOverflow.ellipsis,),
                          const SizedBox(height: 5),
                          Text("Created At : ${formatDate((DateTime.tryParse(order?.order?.createdAt ?? "")?.toLocal() ?? DateTime.now()), [dd, '/', mm, '/', yyyy])}", style: const TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w400,
                            color: Colors.grey,
                          )),
